...
public enum GetFileRequestError {
    // union file_requests.GetFileRequestError (file_requests_apiv2_file_requests_public.stone)
    /**
    ...
    static class Serializer extends UnionSerializer<GetFileRequestError> {
        ...
        @Override
        public void serialize(GetFileRequestError value, JsonGenerator g) throws IOException, JsonGenerationException {
            switch (value) {
        ...
        @Override
        public GetFileRequestError deserialize(JsonParser p) throws IOException, JsonParseException {
            GetFileRequestError value;
            boolean collapsed;
            String tag;
            ...
            if (tag == null) {
                ...
                value = GetFileRequestError.DISABLED_FOR_TEAM;
            }
            else if ("other".equals(tag)) {
                value = GetFileRequestError.OTHER;
            }
            else if ("not_found".equals(tag)) {
                value = GetFileRequestError.NOT_FOUND;
            }
            else if ("not_a_folder".equals(tag)) {
                value = GetFileRequestError.NOT_A_FOLDER;
            }
            else if ("app_lacks_access".equals(tag)) {
                value = GetFileRequestError.APP_LACKS_ACCESS;
            }
            else if ("no_permission".equals(tag)) {
                value = GetFileRequestError.NO_PERMISSION;
            }
            else if ("email_unverified".equals(tag)) {
                value = GetFileRequestError.EMAIL_UNVERIFIED;
            }
            else if ("validation_error".equals(tag)) {
                value = GetFileRequestError.VALIDATION_ERROR;
            }
            else if ("no_write_permission".equals(tag)) {
                value = GetFileRequestError.NO_WRITE_PERMISSION;
            }
            else {
