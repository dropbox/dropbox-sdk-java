...
public final class SharePathError {
    // union sharing.SharePathError (sharing_apiv2_sharing_folders_types.stone)
    ...
    /**
     * Returns {@code true} if this instance has the tag {@link Tag#IS_VAULT},
     * {@code false} otherwise.
     *
     * @return {@code true} if this instance is tagged as {@link Tag#IS_VAULT},
     *     {@code false} otherwise.
     */
    ...
    /**
     * Returns {@code true} if this instance has the tag {@link
     * Tag#IS_VAULT_LOCKED}, {@code false} otherwise.
     *
     * @return {@code true} if this instance is tagged as {@link
     *     Tag#IS_VAULT_LOCKED}, {@code false} otherwise.
     */
    public boolean isIsVaultLocked() {
        return this._tag == Tag.IS_VAULT_LOCKED;
    }

    /**
     * Returns {@code true} if this instance has the tag {@link Tag#IS_FAMILY},
     * {@code false} otherwise.
     *
     * @return {@code true} if this instance is tagged as {@link Tag#IS_FAMILY},
     *     {@code false} otherwise.
     */
    ...
    static class Serializer extends UnionSerializer<SharePathError> {
        ...
        public void serialize(SharePathError value, JsonGenerator g) throws IOException, JsonGenerationException {
            switch (value.tag()) {
                ...
                case IS_VAULT: {
                    g.writeString("is_vault");
                    break;
                }
                case IS_VAULT_LOCKED: {
                    g.writeString("is_vault_locked");
                    break;
                }
                case IS_FAMILY: {
                    g.writeString("is_family");
                    break;
                }
                default: {
                    g.writeString("other");
        ...
        public SharePathError deserialize(JsonParser p) throws IOException, JsonParseException {
            ...
            else if ("is_vault".equals(tag)) {
                value = SharePathError.IS_VAULT;
            }
            else if ("is_vault_locked".equals(tag)) {
                value = SharePathError.IS_VAULT_LOCKED;
            }
            else if ("is_family".equals(tag)) {
                value = SharePathError.IS_FAMILY;
            }
            else {
