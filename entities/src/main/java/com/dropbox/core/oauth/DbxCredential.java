            ...
            while (parser.getCurrentToken() == JsonToken.FIELD_NAME) {
                ...
                try {
                    if (fieldName.equals("access_token")) {
                        accessToken = JsonReader.StringReader.readField(parser, fieldName, accessToken);
                    }
                    else if (fieldName.equals("expires_at")) {
                        expiresAt = JsonReader.Int64Reader.readField(parser, fieldName, expiresAt);
                    }
                        ...
                        appKey = JsonReader.StringReader.readField(parser, fieldName, appKey);
                    }
                    else if (fieldName.equals("app_secret")) {
                        appSecret = JsonReader.StringReader.readField(parser, fieldName, appSecret);
                    }
                    else {
